...
class DataFrameQuerySet(QuerySet):
    ...
    def to_dataframe_local(self, *fields, **kwargs):
        recs = np.core.records.fromrecords(self.values_list(*fields), names=fields)
        return pd.DataFrame.from_records(recs, coerce_float=True)
