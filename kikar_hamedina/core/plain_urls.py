...
urlpatterns = patterns('',
    ...
    url(r'^add-tag/(?P<id>\d+)/$', plain_views.add_tag,
        name='add-tag'),
    url(r'^party/(?P<id>\d+)/$', plain_views.StatusFilterUnifiedView.as_view(),
        kwargs={'variable_column': 'feed__person__party__id',
                'context_object': 'party'},
        name='plain-party'),
    url(r'^person/(?P<id>\d+)/$', plain_views.StatusFilterUnifiedView.as_view(),
    ...
    url(r'^tag/(?P<search_field>\w+)/(?P<id>\w+)/$', plain_views.StatusFilterUnifiedView.as_view(),
