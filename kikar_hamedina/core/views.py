...
def status_update(request, status_id):
    ...
    try:
        ...
        try:
            ...
            dafault_none = lambda x: '' if not x else "{:,}".format(x)
            response_data['likes'] = dafault_none(status.like_count)
            response_data['comments'] = dafault_none(status.comment_count)
            response_data['shares'] = dafault_none(status.share_count)
            response_data['id'] = status.status_id
